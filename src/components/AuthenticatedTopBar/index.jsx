...
const AuthenticatedTopBar = ({ user }) => {
      ...
      <div className="auth-container">
        ...
        <div className="auth-actions">
          ...
          <div className="user-wrapper">
            <button className="avatar-btn" onClick={() => setShowMenu((p) => !p)}>
              {user?.avatar ? <img src={user.avatar} alt="avatar" /> : <span>{user?.name?.[0] || "U"}</span>}
            </button>
            {showMenu && (
              <div className="user-dropdown">
                <button onClick={() => navigate("/profile")}> <FiUser /> Profile</button>
                <button onClick={() => navigate("/messages")}> <FiMessageSquare /> Messages</button>
                <button onClick={() => navigate("/settings")}> <FiSettings /> Settings</button>
                <button onClick={() => {/* TODO logout */}}> <FiLogOut /> Logout</button>
              </div>
            )}
          </div>
        </div>
